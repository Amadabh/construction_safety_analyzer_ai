...
MACHINERY_LABELS = {"Excavator", "Wheel Loader", "Machinery", "Dump Truck"}
...
class RiskAssessor:
    ...
    def assess(self, detections: list[Detection], regulations: list[Regulation] = None) -> RiskAssessment:
        ...
        try:
            ...
            return self._parse_response(data, violations_found)
        except Exception as e:
            print(f"⚠ Risk assessment failed: {e}, falling back to rule-based scoring")
            return self._rule_based_fallback(violations_found)

    def _parse_response(self, data: dict, violations_found: list) -> RiskAssessment:
        violations = [
            Violation(
                type=v["type"],
